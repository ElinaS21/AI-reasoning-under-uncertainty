...
public class Main {
	...
	private static void buildNetwork() {
		...
		for(int i=0;i<bn.nodes.size();i++) 
			if(bn.nodes.get(i).type == 2) { //B
				...
				for(int j=0;j<bn.nodes.size();j++) 
					if(bn.nodes.get(j).type == 1 && (((F_node)bn.nodes.get(j)).vertex==v1 || ((F_node)bn.nodes.get(j)).vertex==v2)){//parent
						bn.nodes.get(j).children.add(bn.nodes.get(i));
						bn.nodes.get(i).parents.add(bn.nodes.get(j));
					}
				//find children and connect
					else if(bn.nodes.get(j).type == 3 && (((E_node)bn.nodes.get(j)).vertex==v1 || ((E_node)bn.nodes.get(j)).vertex==v2)){//child
		...
		for (int i=0; i< bn.nodes.size();i++) { 
			int last = bn.nodes.get(i).probabilityTable[0].length-1;
			double sum=0;
			for(int j=0;j<bn.nodes.get(i).probabilityTable.length; j++) {//sum
				sum += bn.nodes.get(i).probabilityTable[j][last];
			}
			for(int j=0;j<bn.nodes.get(i).probabilityTable.length; j++) {//update
				bn.nodes.get(i).probabilityTable[j][last] /= sum;
			}	
		}
		*/
	...
	private static void probabilisticReasoning(FileWriter writer, LinkedList<Evidence> evidenceList) throws IOException{
		...
		int n;
        System.out.println("\nPlease enter an action:");
        System.out.println("1 for print probability for each of the vertices (enumeration)");  //print format P(A=t) , P(A=f)
        ...
        if(n==1) { //enumeration
            ...
            for(int i=0;i<topologicalList.size();i++) {
                t=enumerationAsk(topologicalList.get(i),evidenceList,copyBN,(LinkedList<Node>)topologicalList.clone());
			...
			System.out.println("Please enter a path (set of vertexes) to get a probability it is free from blockages. example: 1,2,3"); 
			String s = scan.next();
...
//            }
            ...
            System.out.println("Please enter an init vertex (starting from 1) to get a path with highest probability to be free from blockages");
            int init = scan.nextInt();
            System.out.println("Please enter goal vertex (starting from 1) to get a path with highest probability to be free from blockages");
        	...
        	System.out.println("You were asked to type 1/2/3/4...");
	}
	...
	private static double calcPathProbability(LinkedList<Integer> path, LinkedList<Evidence> evidenceList, BayesianNetwork bNet, LinkedList<Node> topologicalList){
		globalInt = 0;
		...
		for(int i=0;i<path.size()-1;i++) {
			if(vertexMatrix[path.get(i)-1][path.get(i+1)-1]==null && vertexMatrix[path.get(i+1)-1][path.get(i)-1]==null)
			{
		...
		for(int i=0;i<path.size()-1;i++) { // i - index in path
			...
			if(inEvidence==1) {  //blockage
				...
				for(int j=0;j<topologicalList.size();j++) {
					...
					if(topologicalList.get(j).type==2 && ((B_node)topologicalList.get(j)).name.equals(name)) {
						t=enumerationAsk(
								topologicalList.get(j),
								(LinkedList<Evidence>) updatedEvidenceList.clone(),
								BayesianNetwork.fresh_copy(copyBN),
			...
			//add not blockage of the edge to evidence list and update probability tables (to fit the chain rule)
	...
	private static double calcPathProbabilityBonus(LinkedList<Integer> currPathCopy, int init, int goal, LinkedList<Evidence> evidenceList, BayesianNetwork bNet, LinkedList<Node> topologicalList) {
		...
		for (Evidence e : evidenceList) {
			...
			if(e.type==2 && node.type==2)//B
				if(((BlockageEvidence)e).edgeName.equals(((B_node)node).name))
					if(e.isTrue)
						return 1;
					else return 0;			if(e.type==3 && node.type==3)//E
				...
				if(((FloodingEvidence)e).vertex==((F_node)node).vertex) {
					...
					for (Node child : copyBN.nodes) {
						if(child.type==2 && node.children.contains(child)) {//is a child
							...
							double newTable[][] = new double[child.probabilityTable.length-linesToDelete.size()][child.probabilityTable[0].length-1];
	...
	private static double[] enumerationAsk(Node X, LinkedList<Evidence> evidenceList, BayesianNetwork bnetwork, LinkedList<Node> topologicalList) {
